...
def update_signals_in_db(historical_data_thread_array, start_index, end_index, symbol_details_array, this_day_scan_start_timestamp, this_day_scan_end_timestamp, this_day_scan_day_end_timestamp, mode, direction, target_multiplier, volume_multiplier, config_id, db_update_conn, db_update_cursor, result_table):
    max_val_for_one_stock = 50000
    this_date = this_day_scan_start_timestamp[0:10]
    ...
    for i in range(start_index, end_index):
        ...
        if signal_index != -1:
            ...
            for index in range(signal_index, scan_end_index + 1):
                if sdf['open'][index] > sdf['close'][index]:
        ...
        if confirmation_index != -1:
            ...
            max_vol = int(max_val_for_one_stock / position_price)
            if volume > max_vol:
                volume = max_vol
            if mode == "backtest":
                ...
                if profit_loss > 0:
                    profit_loss_dir = 1
            insert_result_query = "insert into " + result_table + " (config_id, symbol, category, date, signal_timestamp, position_timestamp, position_price, volume, direction, stoploss, target, exit_timestamp, exit_price, profit_loss_dir, profit_loss) values('" + str(config_id) + "', '" + symbol_details['symbol'] + "', '" + str(symbol_details['category']) + "', '" + this_date + "', '" + signal_timestamp + "', '" + confirmation_timestamp + "', '" + str(position_price) + "', '" + str(volume) + "', '" + str(direction) + "', '" + str(stoploss) + "', '" + str(target) + "', '" + exit_timestamp + "', '" + str(exit_price) + "', '" + str(profit_loss_dir) + "', '" + str(profit_loss) + "');"
            # print(insert_result_query)
            db_update_cursor.execute(insert_result_query)
